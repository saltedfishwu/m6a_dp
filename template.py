...
# %%
def compileModel(model, x_train, x_test, y_test, y_train, gene, condition, encoding):
    model = model
    x_train = x_train
    x_test = x_test
    y_test = y_test
...
def roc(model, x_test, y_test, gene, condition, encoding):
...
def prcurve(model, x_test, y_test, gene, condition, encoding):
...
def main():
    ...
    history = compileModel(model, x_train, x_test, y_test, y_train, gene, condition, encoding)
    lossplot(history, gene, condition, encoding)
    auc = roc(model, x_test, y_test, gene, condition, encoding)
    prauc =prcurve(model, x_test, y_test, gene, condition, encoding)
    mcc =MCC(model,x_test,y_test)
    acc = ACC(model,x_test,y_test)
    results = np.array([auc,prauc,mcc,acc])
    mtx_path ='/home/yuxuan/dp/storeMatrix/' + gene + '_' + condition + '_' + encoding + '.csv'
